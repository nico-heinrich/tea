...
- `oxidation_level` → FK to `oxidation_level` table (1=White, 2=Yellow, 3=Green, 4=Oolong, 5=Black, 6=Dark). Must always be set.
...
6. Upsert vendor, resolve oxidation_level ID
7. Insert tea row
...
3. **Product type / collection filtering**: Only scrape collections that map to real tea categories. Skip samplers, accessories, teaware, blooming teas, herbal teas.
...
Use `product_type`, `tags`, collection name, or name patterns to determine oxidation level.
